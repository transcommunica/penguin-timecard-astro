...
2. **曜日ごとの働き方を柔軟に切り替えられるようにするため**：たとえば「火曜日は基本的に平日勤務だが、ある週だけは午前勤務にする」というようなイレギュラーな場合でも、あらかじめ登録しておいた勤務パターン（この例では「午前勤務」）を選び直すだけで対応できます。標準勤務時間を勤務パターンとして登録しておくことで、その勤務パターンを基準に残業時間の計算も行われます。
...
![カテゴリー、勤務パターン設定 カテゴリーを新規作成する｜操作画面1](/assets/manual/2026/07/category-list-intro.png)

まず「基本情報」で、カテゴリー名と締め日を設定します。

![カテゴリー、勤務パターン設定 カテゴリーを新規作成する｜操作画面2](/assets/manual/2026/07/category-edit-basic.png)

次に「標準勤務時間」で、曜日ごとに勤務パターンを割り当てます。曜日の行をタップすると、勤務パターンの選択画面が開きます。

![カテゴリー、勤務パターン設定 カテゴリーを新規作成する｜操作画面3](/assets/manual/2026/07/category-pattern-picker.png)

ちょうどよい勤務パターンがない場合は、その場で新しい勤務パターンを作成できます。出勤・退勤・休憩開始・休憩終了の時刻を設定して保存します。

![カテゴリー、勤務パターン設定 カテゴリーを新規作成する｜操作画面4](/assets/manual/2026/07/workpattern-new.png)
...
![カテゴリー、勤務パターン設定 曜日ごとの勤務パターンは「初期設定（デフォルト）」｜操作画面1](/assets/manual/2026/07/punch-pattern-selected.png)
...
![カテゴリー、勤務パターン設定 曜日ごとの勤務パターンは「初期設定（デフォルト）」｜操作画面2](/assets/manual/2026/07/punch-pattern-picker.png)
...
- 丸め設定のヘルプには、具体的な計算例が表示されるようになりました。たとえば15分単位の場合、18:23の打刻は、切り捨てなら18:15、切り上げなら18:30、四捨五入なら18:30に調整されます（丸め処理の詳しい仕組みは「[丸め処理（端数処理）](/manual/rounding/)」マニュアルをご覧ください）。

![カテゴリー、勤務パターン設定 各設定のヘルプ機能について（2026年7月更新）｜操作画面1](/assets/manual/2026/07/category-rounding-help.png)

- 出勤・退勤・休憩開始・休憩終了の各ボタン名称の設定にも説明文が加わり、「たとえば『勤務開始』に変更できます」のように具体例が示されるようになりました。

![カテゴリー、勤務パターン設定 各設定のヘルプ機能について（2026年7月更新）｜操作画面2](/assets/manual/2026/07/category-button-names.png)
...
![カテゴリー、勤務パターン設定 対象オフィスの設定（旧・拠点アクセス設定）｜操作画面1](/assets/manual/2026/07/category-target-office.png)
...
![カテゴリー、勤務パターン設定 対象オフィスの設定（旧・拠点アクセス設定）｜操作画面2](/assets/manual/2026/07/category-office-lock.png)

#### 勤務パターン一覧のヘルプ

勤務パターン一覧画面にも「？」ヘルプアイコンが追加され、次の内容が説明されています。

![カテゴリー、勤務パターン設定 勤務パターン一覧のヘルプ｜操作画面1](/assets/manual/2026/07/workpattern-help-icon.png)
...
![カテゴリー、勤務パターン設定 勤務パターン一覧のヘルプ｜操作画面2](/assets/manual/2026/07/workpattern-help-dialog.png)
